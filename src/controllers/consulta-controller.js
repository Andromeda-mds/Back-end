...
const verificarDisponibilidadeMedico = async (_agenda, data) => {
  ...
  horariosDisponiveis.map((item) => {
    if (item.dia == diaConsulta.toString()) matchDiaTrabalho++;
  });
  if (matchDiaTrabalho < 1) return horarioEstaDisponivel;
  ...
  if (_agenda.horariosPreenchidos != null) {
    await Promise.all(
      _agenda.horariosPreenchidos.map(async (item) => {
        ...
        if (_consulta.data.dia == data.data.dia) matchDiaConsultaConcorrente++;
      })
    );
  }
...
exports.CadastrarConsulta = async (req, res, next) => {
  ...
  try {
    ...
    if(_ficha == null) return res.status(404).send({
